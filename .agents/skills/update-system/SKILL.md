...
|------|-------------|-----------------|
| Skills | `.Codex/skills/` | New/updated slash commands |
| Agents | `.Codex/agents/` | New/updated consultant personas |
| Settings | `.Codex/settings.json`, `.Codex/settings.local.json` | **Shared Codex settings (hooks, permissions)** |
...
| Root docs | `AGENTS.md`, `README.md`, `SETUP.md` | Instructions |

**Critical check:** if `.Codex/settings.json` or `.Codex/settings.local.json` changed, flag it explicitly. New hooks/permissions will prompt the teammate to re-approve on next session (Codex security behavior). Tell them this is expected.
...
SETTINGS
  - .Codex/settings.json or settings.local.json changed. You may be
...
Quickly verify:
- Any new skills listed in AGENTS.md? (If AGENTS.md mentions a skill that doesn't exist locally, something's wrong.)
...
- **First run after cloning:** the teammate will see the SessionStart prompt asking to approve hooks from `.Codex/settings.json`. That's normal.
