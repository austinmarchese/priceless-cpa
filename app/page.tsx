...
const servicePages = [
  { title: 'Crypto Accounting & Advising', href: '/crypto-accounting' },
  { title: 'Tax Strategy & Mitigation Planning', href: '/tax-strategy' },
  { title: 'Business Accounting Services', href: '/business-accounting' },
  { title: 'Tax Preparation', href: '/tax-preparation' },
]
...
export default function Home() {
  return (
    <main className="min-h-screen">
      {/* Navigation */}
      <nav className="fixed top-0 left-0 right-0 z-50 bg-bg-deep/90 backdrop-blur-md border-b border-gold/10">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="relative group hidden md:block">
              <button className="text-text-light hover:text-gold transition text-sm">SERVICES</button>
              <div className="absolute top-full left-0 mt-2 w-64 bg-bg-card rounded-lg border border-gold/20 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 shadow-xl">
                {servicePages.map((service) => (
                  <Link key={service.href} href={service.href} className="block px-4 py-3 text-text-light hover:text-gold hover:bg-bg-card-hover transition text-sm first:rounded-t-lg last:rounded-b-lg">
                    {service.title}
                  </Link>
                ))}
              </div>
            </div>
            <Link href="https://calendly.com/pricelesscpa/intro" target="_blank" className="bg-gold hover:bg-gold-light text-bg-deep px-5 py-2 rounded-lg font-semibold text-sm transition">
              Book a Call
            </Link>
          </div>
        </div>
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-20">
        ...
        <div className="absolute inset-0 bg-bg-deep/70"></div>
        <div className="relative z-10 text-center px-6 max-w-4xl mx-auto animate-fade-up">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center mt-10">
            <Link href="https://calendly.com/pricelesscpa/intro" target="_blank" className="bg-gold hover:bg-gold-light text-bg-deep px-8 py-4 rounded-lg font-semibold text-lg transition transform hover:scale-105">
              Book a Call
            </Link>
            <Link href="https://wa.me/13057078959" target="_blank" className="border-2 border-gold text-gold hover:bg-gold hover:text-bg-deep px-8 py-4 rounded-lg font-semibold text-lg transition">
              Chat with me
            </Link>
          </div>
        </div>
      ...
      <section className="py-20 px-6 bg-bg-primary">
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center gap-4 justify-center mb-6">
            <div className="h-px bg-gold/30 flex-1 max-w-32"></div>
            <h2 className="font-display text-3xl md:text-4xl text-center text-gold">Why work with us?</h2>
            <div className="h-px bg-gold/30 flex-1 max-w-32"></div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
            {services.map((service, i) => (
              <div key={i} className="bg-bg-card rounded-xl p-6 text-center border border-gold/10 hover:border-gold/30 transition group">
                <div className="text-4xl mb-4">{service.icon}</div>
                <div className="h-px bg-gold/20 mb-4"></div>
      ...
      <section className="py-20 px-6 bg-bg-deep">
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center gap-4 justify-center mb-12">
            <div className="h-px bg-gold/30 flex-1 max-w-32"></div>
            <h2 className="font-display text-3xl md:text-4xl text-center text-gold">Who WE ARE?</h2>
            <div className="h-px bg-gold/30 flex-1 max-w-32"></div>
          </div>
          ...
          <div className="mt-16 bg-bg-card rounded-2xl p-8 md:p-12 border border-gold/20 text-center max-w-2xl mx-auto">
            ...
            <Link href="https://calendly.com/pricelesscpa/intro" target="_blank" className="inline-block bg-gold hover:bg-gold-light text-bg-deep px-8 py-4 rounded-lg font-semibold transition transform hover:scale-105">
              Book a Call
            </Link>
          </div>
        </div>
      ...
      <section className="py-20 px-6 bg-bg-primary">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex items-center gap-4 justify-center mb-12">
            <div className="h-px bg-gold/30 flex-1 max-w-32"></div>
            <h2 className="font-display text-3xl md:text-4xl text-center text-gold">Our Services</h2>
            <div className="h-px bg-gold/30 flex-1 max-w-32"></div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {servicePages.map((service, i) => (
              <Link key={i} href={service.href} className="bg-bg-card rounded-xl p-8 border border-gold/10 hover:border-gold/30 hover:bg-bg-card-hover transition group">
                ...
                <p className="text-text-muted mt-2">Learn more →</p>
              </Link>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 px-6 bg-bg-deep">
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center gap-4 justify-center mb-12">
            <div className="h-px bg-gold/30 flex-1 max-w-32"></div>
            <h2 className="font-display text-3xl md:text-4xl text-center text-gold">What Clients Say</h2>
            <div className="h-px bg-gold/30 flex-1 max-w-32"></div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {testimonials.map((testimonial, i) => (
              <div key={i} className="bg-bg-card rounded-xl p-6 border border-gold/10">
      ...
      {/* Footer */}
      <footer className="py-12 px-6 bg-bg-primary border-t border-gold/10">
        <div className="max-w-6xl mx-auto text-center">
          ...
          <div className="flex justify-center gap-6 mt-4">
            <Link href="https://calendly.com/pricelesscpa/intro" target="_blank" className="text-gold hover:text-gold-light transition text-sm">Book a Call</Link>
            <Link href="https://wa.me/13057078959" target="_blank" className="text-gold hover:text-gold-light transition text-sm">WhatsApp</Link>
          </div>
        </div>
      </footer>
    </main>
